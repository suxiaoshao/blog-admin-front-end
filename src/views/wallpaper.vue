    ...
    export default {
        ...
        methods: {
            get_base() {
                ...
                this.url = '';
                this.real_url = '';
                this.axios.get("http://192.168.0.103:8082/api/wallpaper/base").then(response => {
                    if (response.data.success) {
                        this.url = response.data.data;
                        let url_list = this.url.split("/");
                        this.real_url =
                            "http://192.168.0.103:8082/img/wallpaper/" + url_list[url_list.length - 2] + "===" + url_list[url_list.length - 1];
                        this.loading = false
                    } else {
                        this.loading = false;
                        this.$notify({
            ...
            post_wallpaper(ok) {
                this.loading = true;
                this.axios.post("http://192.168.0.103:8082/api/wallpaper/base", {
                    ok: ok,
                    url: this.url
                }).then(response => {
                    if (response.data.success) {
